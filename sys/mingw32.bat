...
echo %PATH%
cd radare2
sh.exe -c "export PATH=/c/mingw/bin:/c/mingw/msys/1.0/bin:${PATH} ; gcc -v"
sh.exe -c "uname | tr 'A-Z' 'a-z'"
sh.exe -c "echo ${CC}"
sh.exe -c "export PATH=/c/mingw/bin:/c/mingw/msys/1.0/bin:${PATH} ; ./configure --with-ostype=mingw32 --build=i686-unknown-windows-gnu ; mingw32-make -j1 ; mingw32-make w32dist USE_ZIP=NO"
if "%APPVEYOR%" == "True" (
